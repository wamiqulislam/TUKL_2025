...
Original file is located at
    https://colab.research.google.com/drive/10n2YbSOuXkZtG6mF-p5rvu6W-R4gYDQE
"""
...
def log_iteration_to_file(strategy, history, iteration, initial_size, query_size, iters, total_epochs, val_loader, model, device):
    current_samples = history["samples"][-1]
    epoch_data = history["epoch_metrics"][-1]
    ...
    f1_per_class = sklearn.metrics.f1_score(y_true_all, y_pred_all, average=None).tolist()
    cm = confusion_matrix(y_true_all, y_pred_all).tolist()

    total_samples = total_labeled_samples(initial_size, iters, query_size)
    effective_samples = eff_total_labeled_samples(initial_size, iters, query_size)
    ...
    log_data = {
        "settings": {
            ...
            "total_epochs_per_round": total_epochs,
            "total_labeled_samples": total_samples,
            "effective_total_labeled_samples": effective_samples,
        },
        "iteration": iteration + 1,
        "current_labeled_samples": current_samples,
        "epoch_metrics": epoch_data,
        "final_epoch_summary": {
            "train_loss": final_epoch["train_loss"],
            "train_acc": final_epoch["train_acc"],
            "train_avg_f1": final_epoch["train_avg_f1"],
            "val_loss": final_epoch["val_loss"],
            "val_acc": final_epoch["val_acc"],
            "val_avg_f1": final_epoch["val_avg_f1"]
        },
        "f1_per_class": f1_per_class,
        "confusion_matrix": cm
    }

    filename = f"{strategy}_{total_samples}.txt"
...
def active_learning_loop(model, labeled_dataset, unlabeled_dataset, val_dataset, strategy, device, iters=10, query_size=100, final_epochs=10, loss_mode = 'ce', batch_size = 16):
    ...
    for it in range(iters):
        ...
        initial_labeled_size = len(labeled_dataset.X) - it * query_size
        ...
        log_iteration_to_file(
            strategy=strategy,
            history=history,
            iteration=it,
            initial_size=initial_labeled_size,
            query_size=query_size,
            iters=iters,
            total_epochs=epochs,
